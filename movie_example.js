...
function getMovieTemplate(movie){
 ...
 //   $template.find('.vote_count').text(movie.vote_count);
    return $template;
}
...
function getMovieParams() {
    ...
    const ratingScore = getMovieTemplate()
    ...
    const sortBy = $('#filter-score').val();
    const params = {...timeRange, with_score: score.join(), sort_by: sortBy}
